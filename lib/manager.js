...
class AudioManager {
  ...
  constructor (sourceType, options, capture = false) {
    ...
    this.analyser = new AudioAnalyser(this.options.bufferSize, this.options.analyserDeepmergeOptions ? this.options.analyser : options.analyser, this.options.analyserDeepmergeOptions);
  ...
  /**
   * creates a copy of the available buffer, if available, starting at start of a duration duration, unit: MS
   * 
   * @param {number} start the offset timer in milliseconds ms from left from which to begin the copy 
   * @param {number} duration the duration in milliseconds of the copy
   * 
   * @return {Blob|false} the blob if source can be cut (must be a file), false otherwise
   */
  getAudioCopy (start, duration) {
    if (this.sourceType === AudioManager.SOURCE_TYPE.FILE) {
      return this.audioSource.copyAudioData(start, duration);
    } else {
      return false;
    }
  }
